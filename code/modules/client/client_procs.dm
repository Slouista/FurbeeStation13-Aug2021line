	...
	if(!query_client_in_db.NextRow())
		if (CONFIG_GET(flag/panic_bunker) && !holder && !GLOB.deadmins[ckey] && !(ckey in GLOB.bunker_passthrough))
			...
			if(!query_client_in_db_whitelist.NextRow()) //they are not on the whitelist OR have played before! on the database!
				...
				to_chat(src, "<span class='notice'>You must first join the Discord to verify your account before joining this server.<br>To do so, read the rules and post a request in the #whitelist-application under the \"Main server\" category in the Discord server linked here: <a href='https://discord.gg/69WsShCdxa'>https://discord.gg/69WsShCdxa</a></span>") //CIT CHANGE - makes the panic bunker disconnect message point to the discord
