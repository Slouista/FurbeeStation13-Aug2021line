...
/client/proc/addbunkerbypass(ckeytobypass as text)
	...
	//var/sql_ckey = sanitizeSQL(ckeytobypass) //seems like we don't need this as we use rust_g for sql
	var/datum/DBQuery/query_client_in_db = SSdbcore.NewQuery("SELECT 1 FROM [format_table_name("whitelist")] WHERE ckey = '[ckeytobypass]'")
	if(!query_client_in_db.Execute())
		qdel(query_client_in_db)
		return
	if(!query_client_in_db.NextRow())
		//add to database!
		var/datum/DBQuery/query_add_player_whitelist = SSdbcore.NewQuery("INSERT INTO [format_table_name("whitelist")] (`ckey`) VALUES ('[ckeytobypass]')")
